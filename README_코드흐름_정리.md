...
API 키는 README, 테스트 결과, 화면 응답에 기록하지 않습니다.
...
1. `session_id`로 State를 가져옵니다.
2. 전달된 Profile과 관심 분야를 정규화해 먼저 저장합니다.
3. UI 이벤트에 따라 이전 Clarify와 임시 결과를 정리합니다.
4. 추가 질문 제출이면 답변을 정책별로 저장합니다.
5. `agent.handle_turn()`을 호출합니다.
6. 응답 텍스트와 화면 렌더링에 필요한 State만 반환합니다.
...
③ 명시적 자연어 패턴 / UI 이벤트 라우팅
    ↓
④ 애매한 문장만 OpenAI Prompt A로 Intent 분석
    ↓
...
| `CLARIFY_ADDITIONAL` | 정책별 세부 조건이 부족함 | 최대 3개 추가 질문 카드 |

Clarify 도중 `추천해줘`, `설명해줘`, `자격 확인해줘` 같은 명확한 새 요청이 들어오면 이전 질문 상태를 정리하고 새 task로 전환합니다. `이 정책 말고 다른 정책`, `나 자격 다른 거 확인하고 싶어`처럼 기존 대상을 배제하는 문장은 `focus_policy_id`와 `selected_policy_id`를 지운 뒤 `CLARIFY_POLICY` 정책 선택 카드로 이동합니다. 단, `예`, `아니오`, 날짜, 구조화 답변은 현재 질문에 대한 답으로 처리합니다.
