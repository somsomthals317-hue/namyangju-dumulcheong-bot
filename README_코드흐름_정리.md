...
- 한 화면에 최대 3개
- 질문 ID 기준으로 정책별 저장
- 자연어 한 문장에 여러 답이 있으면 현재 화면의 질문별로 분리하여 저장
- 예/아니오 선택형 답은 카드와 같은 enum으로 정규화하고, 현재 질문 ID와 공식 선택지 밖의 모델 출력은 폐기
- 답변 후 같은 ELIGIBILITY step 재실행
- 최종 결과에서 추가 답변 재입력 가능

Profile 카드에 자연어로 답할 때도 한 문장에서 확인되는 값을 모두 저장합니다. 예를 들어
“만 24세고 남양주시에 거주하고 있어”는 `age=24`, `residency=예`로 함께
구조화됩니다. 자주 쓰는 표현은 코드가 먼저 파싱하고, 부족한 필드만 GPT가
보조 추출합니다. GPT 값도 UI의 허용 enum으로 다시 검증하므로 입력 방식에 따라
자격 결과가 달라지지 않습니다.
...
3. GPT step과 코드 step을 Task별로 병합합니다. 현재 문장에 명시된 공식 정책명과 관심 분야는 이전 State에서 가져온 대상보다 우선합니다.
   사용자가 명시한 “조건 없이/프로필 없이”도 모델의 `false`보다 우선합니다.
...
같은 발화의 뒤 step이 “내가 자격되는지도”, “그 정책 자격도”처럼 앞 step을
가리키면 세션의 오래된 정책이 아니라 **같은 발화에서 먼저 확정한 정책 ID**를
이어받습니다. 예를 들어 “입영지원금 설명해주고 내가 자격되는지도 확인해줘”는
`EXPLAIN(입영지원금)`과 `FOLLOW_UP + ELIGIBILITY(입영지원금)`로 고정됩니다.
`자격도/자격을/자격이`처럼 조사가 붙은 표현도 ELIGIBILITY 동사로 인식합니다.
...
설명+추천+자격처럼 세 Task가 함께 있으면 중간 카드에서 이미 준비된 결과와
앞으로 남은 Task를 모두 알려줍니다. 따라서 추천 입력을 기다리는 동안 자격 확인이
대기 중이라는 사실이 사라지지 않으며, 마지막에는 세 결과를 요청 순서대로 묶습니다.
...
| 연속 대화·복합 Task·탐색 모드 복구 | 17 | GPT Task 누락 복구, 분야형 설명 정책 선택 → Profile/조건 없이 버튼 → 합본 응답, 추천 후 다른 분야 전환, 자연어 Profile·다중 추가답변, 조사 포함 자격 표현, 같은 발화 정책 참조, 3-Task 대기 안내 | PASS |
